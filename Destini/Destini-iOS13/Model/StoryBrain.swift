...
struct StoryBrain {
    ...
    func getHeadText() -> String {
        return example[number].storyTitle
    }
    
    func getButton1() -> String {
        return example[number].choice1
    }
    
    func getButton2() -> String {
        return example[number].choice2
    }
    
    mutating func nextStory(input : String) {
        if (input == example[number].choice1) {
            number = example[number].choice1Destination
        }
        if (input == example[number].choice2) {
            number = example[number].choice2Destination
        }
    }
